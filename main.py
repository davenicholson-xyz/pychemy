...
def parse_args():
    ...
    p.add_argument('-k', '--api-key',        metavar='KEY',  help='Wallhaven API key')
    p.add_argument('-u', '--username',       metavar='USER', help='Wallhaven username')
    p.add_argument('-q', '--query',          metavar='Q',    help='Default search query')
    p.add_argument('-c', '--categories',     metavar='MASK', help='Category bitmask (e.g. 111)')
    p.add_argument('-p', '--purity',         metavar='MASK', help='Purity bitmask (e.g. 100)')
    p.add_argument('-t', '--thumb-size',     dest='thumb-size',     metavar='SIZE', choices=['xs', 'sm', 'm', 'l', 'xl'], help='Thumbnail size')
    p.add_argument('-r', '--min-resolution', dest='min-resolution', metavar='RES',  help='Minimum resolution (e.g. 1920x1080)')
    p.add_argument('-s', '--script',         metavar='PATH', help='Script to run on selected wallpaper')
    p.add_argument('-x', '--close-on-select', dest='close-on-select', action='store_true', default=None, help='Close after selecting wallpaper')
    sort_group = p.add_mutually_exclusive_group()
    sort_group.add_argument('-H', '--hot',    dest='sorting', action='store_const', const='hot',        help='Start on Hot sorting')
    sort_group.add_argument('-l', '--latest', dest='sorting', action='store_const', const='date_added', help='Start on Latest sorting')
    sort_group.add_argument('-T', '--top',    dest='sorting', action='store_const', const='toplist',    help='Start on Toplist sorting')
    sort_group.add_argument('-R', '--random', dest='sorting', action='store_const', const='random',     help='Start on Random sorting')
    p.add_argument('--search',     dest='query',      metavar='QUERY', help='Start with this search query')
    p.add_argument('-C', '--collection', dest='collection', metavar='NAME',  help='Start with this collection open')
    p.add_argument('-o', '--output', action='store_true', default=None, help='Print wallpaper filepath to stdout after setting')
